...
class Player
    ...
    def attacks(other_player)
        puts "\n Le joueur #{@name} attaque le joueur #{other_player.name} ! "
        damage = compute_damage.to_i
        puts "  ...\nPif! Pam! Poum!\n ...\n==> #{@name} vient d'infliger #{damage} points de dommages à #{other_player.name} !"
...
class HumanPlayer < Player
    ...
    def show_state
        puts "Joueur #{@name} : #{@life_points} points de vie et une arme de niveau #{@weapon_level}"
    end
    ...
    def search_health_pack
        ...
        if power_new_weapon > 1 && power_new_weapon < 6
            puts "Bravo, tu as trouvé un pack de +50 points de vie !"
            @life_points += 50
            @life_points = 100 if @life_points > 100
        elsif power_new_weapon == 6
